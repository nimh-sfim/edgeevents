...
switch f_dattype,
    ...
    case 'REAL',
        ...
        L = n*mylog((alpha-1)/xmin) - alpha.*sum(mylog(z./xmin));
    ...
    case 'INTS',
        ...
        for xm=1:length(xmins)
            ...
            if fcatch==0
                try
                    ...
                    zdiff = sum( repmat(double(1:xmin-1)',1,length(vec)).^-repmat(vec,xmin-1,1) ,1);
                    L = -vec.*sum(mylog(z)) - n.*mylog(zvec - zdiff);
                catch
            ...
            if fcatch==1
                ...
                L       = -Inf*ones(size(vec));
                slogz   = sum(mylog(z));
                xminvec = double(1:xmin-1);
                for k=1:length(vec)
                    L(k) = -vec(k)*slogz - n*mylog(zvec(k) - sum(xminvec.^-vec(k)));
                end
            end;
            [Y,I] = max(L);
            % compute KS statistic
            fit = cumsum(((double(xmin:xmax).^-vec(I)))./ (zvec(I) - sum(double(1:xmin-1).^-vec(I))));
            cdi = cumsum(hist(z,double(xmin:xmax))./n);
        ...
        L     = -alpha*sum(mylog(z)) - n*mylog(zvec(find(vec<=alpha,1,'last')) - sum(double(1:xmin-1).^-alpha));

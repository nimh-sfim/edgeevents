...
switch f_dattype,
    ...
    case 'INTS',
        ...
        L  = -Inf*ones(size(vec));
        for k=1:length(vec)
            L(k) = -vec(k)*sum(mylog(z)) - nz*mylog(zvec(k) - sum(double(1:xmin-1).^-vec(k)));
        end
        [Y,I] = max(L);
        alpha = vec(I);

        fit = cumsum(((double(xmin:xmax).^-alpha))./ (zvec(I) - sum(double(1:xmin-1).^-alpha)));
        cdi = cumsum(hist(z,double(xmin:xmax))./nz);
        gof = max(abs( fit - cdi ));
        pz  = nz/N;

        mmax = 20*xmax;
        pdf = [zeros(xmin-1,1); ((double(xmin:mmax).^-alpha))'./ (zvec(I) - sum(double(1:xmin-1).^-alpha))];
            ...
            for qm=1:length(qmins)
                ...
                if nq>1
                    try
                        ...
                        zdiff = sum( repmat(double(1:qmin-1)',1,length(vec)).^-repmat(vec,qmin-1,1) ,1);
                        L = -vec.*sum(mylog(zq)) - nq.*mylog(zvec - zdiff);
                    catch
                       % iterative version (more memory efficient, but slower)
                       L       = -Inf*ones(size(vec));
                       slogzq  = sum(mylog(zq));
                       qminvec = double(1:qmin-1);
                       for k=1:length(vec)
                           L(k) = -vec(k)*slogzq - nq*mylog(zvec(k) - sum(qminvec.^-vec(k)));
                       end;
                    end;
                    [Y,I] = max(L);

                    fit = cumsum(((double(qmin:qmax).^-vec(I)))./ (zvec(I) - sum(double(1:qmin-1).^-vec(I))));
                    cdi = cumsum(hist(zq,double(qmin:qmax))./nq);
...
% overwrite log to work on double
function [out] = mylog(x)
    out = log(double(x)) ; 
end

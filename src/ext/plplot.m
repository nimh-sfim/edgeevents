...
switch f_dattype,
    ...
    case 'REAL',
        ...
        xr  = [10.^floor(mylog10(min(x))) 10.^ceil(mylog10(max(x)))];
        xrt = (round(mylog10(xr(1))):2:round(mylog10(xr(2))));
        if length(xrt)<4, xrt = (round(mylog10(xr(1))):1:round(mylog10(xr(2)))); end;
        yr  = [10.^floor(mylog10(1/n)) 1];
        yrt = (round(mylog10(yr(1))):2:round(mylog10(yr(2))));
        if length(yrt)<4, yrt = (round(mylog10(yr(1))):1:round(mylog10(yr(2)))); end;
    ...
    case 'INTS',
        ...
        n = length(x);        
        q = double(unique(x));
        c = hist(double(x),double(q))'./n;
        c = [[q; q(end)+1] 1-[0; cumsum(c)]]; c(c(:,2)<10^-10,:) = [];
        cf = (double(xmin:q(end))'.^-alpha)./(zeta(alpha) - sum(double(1:xmin-1).^-alpha));
        cf = [double((xmin:q(end)+1)') 1-[0; cumsum(cf)]];

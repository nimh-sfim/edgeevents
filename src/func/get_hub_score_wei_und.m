...
% lower hubscore (higer rank) corresponds to higher likelihood of hubness
...
switch lower(hubdeff)
    ...
    case 'bassett2008' 
        ...
        eg = eigenvector_centrality_und(double(mat)) ;
    ...
    case 'zuo2011'
        % 'functional' hubs, riff on original paper
        d = degrees_und(mat>0) ;
        s = strengths_und(abs(mat)) ;
        % subgraph centrality
        sg = my_sg_cent(mat) ; 
        eg = eigenvector_centrality_und(double(mat)) ;
...
if hubrank
    ...
    hubscore = mean(rank_meas,2,'omitnan') ; % average to get hubscore
